...
def test_autogm_scene_generate_places(mock_autogm, mock_party, mocker):
    ...
    @pytest.fixture
    def mock_campaign(mock_party):
        campaign = Campaign(
            name="Test Campaign",
            description="A test campaign",
            world=mock_party.world,
            episodes=[],
            players=[],
            associations=[],
            summary="",
    ...
    def test_campaign_creation(mock_campaign):
        assert mock_campaign.name == "Test Campaign"
        assert mock_campaign.description == "A test campaign"
        assert mock_campaign.world.name == "Test World"
        assert mock_campaign.episodes == []
        assert mock_campaign.players == []
        assert mock_campaign.associations == []
        assert mock_campaign.summary == ""
        assert mock_campaign.outline == []
        assert mock_campaign.side_quests == []
        assert mock_campaign.current_episode is None

    def test_campaign_add_episode(mock_campaign):
        episode = mock_campaign.add_episode(
            name="Episode 1", description="First episode"
        )
        assert episode.name == "Episode 1"
        assert episode.description == "First episode"
        assert episode in mock_campaign.episodes

    def test_campaign_update_episode(mock_campaign):
        episode = mock_campaign.add_episode(
            name="Episode 1", description="First episode"
        )
        updated_episode = mock_campaign.update_episode(
            pk=episode.pk, name="Updated Episode 1", description="Updated description"
        )
        assert updated_episode.name == "Updated Episode 1"
        assert updated_episode.description == "Updated description"

    def test_campaign_delete_episode(mock_campaign):
        episode = mock_campaign.add_episode(
            name="Episode 1", description="First episode"
        )
        mock_campaign.delete_episode(episode.pk)
        assert episode not in mock_campaign.episodes

    def test_campaign_generate_npcs(mock_campaign, mocker):
        mocker.patch("requests.post", return_value="success")
        npc_data = [
            {
                "species": "Human",
                ...
                "backstory": "Once a farmer, now a hero",
            }
        ]
        mock_campaign.generate_npcs(npc_data)
        assert len(mock_campaign.associations) == 1
        assert isinstance(mock_campaign.associations[0], Character)
        assert mock_campaign.associations[0].name == "John Doe"

    def test_campaign_generate_combatants(mock_campaign, mocker):
        mocker.patch("requests.post", return_value="success")
        combatant_data = [
            {
                "combatant_type": "humanoid",
        ...
        mock_campaign.generate_combatants(combatant_data)
        assert len(mock_campaign.associations) == 1
        assert isinstance(mock_campaign.associations[0], Creature)
        assert mock_campaign.associations[0].name == "Orc Warrior"

    def test_campaign_generate_items(mock_campaign, mocker):
        mocker.patch("requests.post", return_value="success")
        item_data = [
            {
                "rarity": "rare",
                ...
                "attributes": ["Sharp", "Glowing"],
            }
        ]
        mock_campaign.generate_items(item_data)
        assert len(mock_campaign.associations) == 1
        assert isinstance(mock_campaign.associations[0], Item)
        assert mock_campaign.associations[0].name == "Magic Sword"

    def test_campaign_generate_places(mock_campaign, mocker):
        mocker.patch("requests.post", return_value="success")
        place_data = [
            {
                "location_type": "city",
                ...
                "backstory": "Founded centuries ago",
            }
        ]
        mock_campaign.generate_places(place_data)
        assert len(mock_campaign.associations) == 1
        assert isinstance(mock_campaign.associations[0], Location)
        assert mock_campaign.associations[0].name == "Test City"

    def test_campaign_resummarize(mock_campaign, mocker):
        mocker.patch("markdown.markdown", return_value="Formatted summary")
        mocker.patch("requests.post", return_value="success")
        episode = mock_campaign.add_episode(
            name="Episode 1", description="First episode"
        )
        episode.summary = "Summary of episode 1"
        mock_campaign.resummarize()
        assert mock_campaign.summary == "Formatted summary"

    def test_campaign_page_data(mock_campaign):
        data = mock_campaign.page_data()
        assert data["name"] == "Test Campaign"
        assert data["description"] == "A test campaign"
        assert data["summary"] == ""
        assert data["start_date"] == "Unknown"
        assert data["end_date"] == "Ongoing"
