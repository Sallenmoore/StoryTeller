...
class TestFaction:
    def test_create(self, world):
        obj = Faction(name="TestFaction", world=world)
        obj.save()
        assert obj.name == "TestFaction"
        assert obj.pk
        assert obj.user == world.user

    def test_update(self, world):
        for _ in range(3):
            Faction(name=f"TestFaction{_}", world=world).save()
        objs = Faction.all()
        for obj in objs:
            assert obj.world
            obj.name += " --updated"
    ...
    def test_read(self, world):
        ...
        objs = Faction.all()
        for obj in objs:
            ret_obj = Faction.get(obj.pk)
            assert ret_obj

    def test_search(self, world):
        for _ in range(3):
            Faction(name=f"TestFaction{_}", world=world).save()
        objs = Faction.search(world=world)
        assert objs
