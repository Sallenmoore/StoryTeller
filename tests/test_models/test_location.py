...
class TestLocation:
    def test_create(self, world):
        obj = Location(name="TestLocation", world=world)
        obj.save()
        assert obj.name == "TestLocation"
        assert obj.pk
        assert obj.user == world.user

    def test_update(self, world):
        for _ in range(3):
            Location(name=f"TestLocation{_}", world=world).save()
        objs = Location.all()
        for obj in objs:
            assert obj.world
            obj.name += " --updated"
    ...
    def test_read(self, world):
        ...
        objs = Location.all()
        for obj in objs:
            ret_obj = Location.get(obj.pk)
            assert ret_obj

    def test_search(self, world):
        for _ in range(3):
            Location(name=f"TestLocation{_}", world=world).save()
        objs = Location.search(world=world)
        assert objs
