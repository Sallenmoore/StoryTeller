...
class TestCharacter:
    def test_create(self, world):
        obj = Character(name="TestCharacter", world=world)
        obj.save()
        assert obj.name == "TestCharacter"
        assert obj.pk
        assert obj.user == world.user

    def test_update(self, world):
        for _ in range(3):
            Character(name=f"TestCharacter{_}", world=world).save()
        objs = Character.all()
        for obj in objs:
            assert obj.world
            obj.name += " --updated"
    ...
    def test_read(self, world):
        ...
        objs = Character.all()
        for obj in objs:
            ret_obj = Character.get(obj.pk)
            assert ret_obj

    def test_search(self, world):
        for _ in range(3):
            Character(name=f"TestCharacter{_}", world=world).save()
        objs = Character.search(world=world)
        assert objs
