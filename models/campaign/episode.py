...
class Episode(AutoModel):
    ...
    def summarize_transcription(self):
        if self.transcription:
            ...
            prompt = f"Summarize the following transcription of a TTRPG session for a {self.world.genre} world using a snarky and observational tone. The summary should focus on story and narrative rather than game mechanics, highlighting the key elements of the transcription and its significance within the larger world, while leaving out any irrelevant remarks or conversation. Here is some context about the world: {self.world.name}, {self.world.history}. Here is some context about the campaign: {self.campaign.name} [{self.campaign.start_date} - {self.campaign.end_date}], {self.campaign.summary}. Try to use full names for characters, places, and things in the world, where possibible. Here is the transcription: {self.transcription}."
